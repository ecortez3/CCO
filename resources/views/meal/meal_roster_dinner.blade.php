...
@section('content')
		...
		<table class="table-striped">
			...
			@foreach ($clients as $client)
				...
				<tr class="alt">
					<td align="center" width = "10.0%"><a href = "{{ action('MealController@dinnerButton', [$client->id]) }}">{{ $client->id }}</a></td>
					<td align="center" width = "60.0%"><a href = "{{ action('MealController@dinnerButton', [$client->id]) }}">{{ $client->lname }}, {{ $client->fname }}</a></td>
					<td align="center" width = "10.0%">{{ $client->gender }}</a></td>
					<td align="center" width = "10.0%">{{ $client->meal->last()->dinner }}</a></td>
				</tr>	
			@endforeach
		</table>
@stop